...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
	...
	{
		buf2[t2++] = dum2;
		HAL_UART_Transmit(&huart2, buf2 + t2 - 1, 1, 10); // Terminal Echo
...
void TrimEx(char *dest, char *s)  // s = "____x x x__\t_\r\n" ==> "x x x"
{
...
char * Trim(char *s)  // s = "____x x x__\t_\r\n" ==> "x x x"
{
	int h=0,t=strlen(s)-1;
